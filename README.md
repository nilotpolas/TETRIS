...
> *IACR Transactions on Cryptographic Hardware and Embedded Systems (TCHES)*, Vol. 2026, Issue 4  
> **To appear.**
...
Given **both** a latency ceiling and a randomness ceiling, MARC finds the gadget assignment that minimises silicon area (Gate Equivalents). It also computes and optionally displays the full Pareto frontier of non-dominated (latency, randomness, area) designs reachable within the constraint box.

**Use when** you must meet both a throughput requirement and a randomness budget, and you want the smallest possible chip area — the common constraint regime for area-constrained IoT implementations.

```bash
python run_marc.py --design <design>.c \
    --max-latency <cycles> --max-randomness <bits> \
    --order <d> [options]
```
...
| Flag | Description | Default |
|------|-------------|---------|
| `--max-latency N` | Latency ceiling in clock cycles | required |
| `--max-randomness N` | Randomness ceiling in bits | required |
| `--show-pareto` | Print full Pareto frontier table | off |

**Examples:**

```bash
# Boyer-Peralta S-box, order d=1
python run_marc.py \
    --design Boyer_Peraltas_sbox.c \
    --max-latency 6 --max-randomness 120 \
    --order 1

# Canright S-box, order d=1, show all Pareto-optimal designs, skip synthesis
python run_marc.py \
    --design Canright_sbox.c \
    --max-latency 5 --max-randomness 80 \
    --order 1 \
    --show-pareto --skip-synth

# Include COMAR, show Pareto frontier
python run_marc.py \
    --design Canright_sbox.c \
    --max-latency 5 --max-randomness 80 \
    --order 1 --use-comar --show-pareto
```

**Algorithm — 4-D DP + Pareto extraction:**

```
State:  dp[level][bonus_spent][rand_spent][comar_used]  →  minimum area
        Explores every (latency, randomness) operating point within the box.

Pareto: collect all feasible (latency, randomness, area) triples from the
        final DP states; report the non-dominated subset.

Best:   the single min-area point within the full constraint box is synthesised.
```

See [MARC_ALGORITHM.md](MARC_ALGORITHM.md) for full algorithm documentation.
